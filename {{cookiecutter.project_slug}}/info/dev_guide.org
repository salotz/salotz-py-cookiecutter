...
The version number for the package builds is handled through the git
tags and the ~versioneer~ tool. See the next section on this.
...
You will also probably want to update the docs as well for the release
build:
...
**** Publishing the Website & Documentation


Currently the sphinx docs serve as the main web page.

So just make sure you build them and visually "test" them, then you
can deploy easily:

#+begin_src bash
inv py.docs-build # or inv docs.serve
inv py.website-deploy
#+end_src
