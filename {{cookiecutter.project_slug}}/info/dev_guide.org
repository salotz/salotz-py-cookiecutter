...
*** Docker (Application Container) Environments

**** Why?

Application containers are useful in scenarios where LXD containers
are not and vice versa. Application containers are best used when they
are stateless (as a complete POSIXy system) whereas LXD containers
support stateful tasks much better. So if you want to have a complete
environment where you can store scratch work, intermediates etc LXD is
a much better choice. That is why we use it for this purpose.

Application containers like Docker are really meant to run a single
application at a time in an isolated environment. The main goal of
these is to reduce operational complexity that comes with configuring
many different server like apps on a single host. Each app gets its
own host and never has to fight with the other ones. Also many long
running server type softwares distribute docker images which are very
easy to use and install. I.e. there really isn't an install process
beyond ~docker pull~.

For this purpose you may use either Docker or Podman as they are
largely compatible. We suggest using podman as it doesn't require
running a daemon. Obviously Docker is more popular and so it is fine
too.

Running multiple application containers together for some shared
purpose is extremely common and so using a tool that does this on top
of Docker seems almost a necessity so we will also give best practices
for this. We call these *clusters*.
...
You may also need to update the pinned versions of the meta-tooling
sometimes as well (i.e. ~.jubeo/requirements.txt~) which you can do by
editing ~/.jubeo/requirements.in~ (and ~.jubeo/tools.requirements.txt~
if you need to pin the version of ~pip~).

And then run the core command to pin these:

#+begin_src bash
inv core.pin-tool-deps
#+end_src

Because pinning every other env depends on the compile
~/.jubeo/requirements.txt~ this will effect them.

# TODO: this behaviour should change in the future. It should be
# possible to make envs which are not "contaminated" and depend on the
# jubeo tooling.

There are 4 different groups of env specs beyond the meta-tooling
(~jubeo~) env-spec you can be interested in:

- project :: general project management envs like testing and
  building; in the ~envs~ dir (i.e. ~envs/env-name~).
- examples :: for showcasing real use cases; each example
  (~info/examples/example-name~) has its own ~env~ folder for the spec.
- tutorials :: for writing executable tutorials that will be in
  documentation; each tutorial (~info/tutorials/tutorial-name~) has
  its own ~env~ folder for the spec.
- jigs :: general purpose "setups" for various things, each jig
  (~jigs/jig-name~) has its own ~env~ folder for the spec.


The examples, tutorials, and jigs typically have commands in their own
~tasks.py~ which takes care of managing the env. These however, just
end up using the root ~tasks~ for this. I.e.:

To pin the envs:

#+begin_src bash
inv docs.pin-example -n example-name
inv docs.pin-tutorial -n tutorial-name
inv py.pin-jig -n jig-name
#+end_src

To make an env:

#+begin_src bash
inv docs.env-example -n example-name
inv docs.env-tutorials -n tutorials-name
inv py.env-jig -n jig-name
#+end_src

If you omit the name it will perform the actions on all of them e.g.:

#+begin_src bash
inv docs.env-example
#+end_src


You can also always have env-specs and env builds at any arbitrary
path and with any env method (~conda~, ~venv~, ~pyenv~, etc.):

#+begin_src bash
inv env.deps-pin-path --path path/to/env-spec-dir
inv env.make-env \
    --spec 'venv' \
    --spec path/to/env-spec-dir \
    --path path/to/_env-build
#+end_src
...
#+begin_src bash
  inv docs.new-example --name='myexample' --template='org' --env='conda_blank'
  inv docs.new-tutorial  --name='mytutorial --template=jupyter' --env='conda_blank'
#+end_src
...
Then you must first set configuration values in the ~tasks/config.py~
which cannot be inferred or generated (at this time).

See the comments and instructions in the template file for doing this
which has specific instructions for the project type.
