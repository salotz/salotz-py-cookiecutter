...
def bimhaw_bootstrap(cx):
    ...
    with cx.prefix(". tmp/py_bootstrap_env/bin/activate"):
        ...
        cx.run("python -m bimhaw.init "
               "--config ~/.{{cookiecutter.owner_nickname}}.d/bimhaw/config.py "
               "--lib ~/.{{cookiecutter.owner_nickname}}.d/bimhaw/lib")
...
def pyenv_bootstrap(cx):
    with cx.cd("$HOME/.{{cookiecutter.owner_nickname}}.d"):
...
def push_profile(cx):
    ...
    rsync(cx,
          f"{homedir}/.{{cookiecutter.owner_nickname}}.d",
          f"{homedir}/",
